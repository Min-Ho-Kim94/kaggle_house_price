...
```{r}
names(all[,grep("Garage",names(all))])
names(all[,grep("Bsmt",names(all))])
names(all[,grep("Bath",names(all))])
names(all[,grep("Pool",names(all))])
names(all[,grep("Lot",names(all))])
names(all[,grep("Exter",names(all))])
names(all[,grep("Fire",names(all))])
names(all[,grep("Flr",names(all))])
names(all[,grep("Porch",names(all))])
```
...
```{r}
gbgrid <- expand.grid(n.trees=(2:10)*50,
...
```{r}
gb_model1 <- train(SalePrice~., data=train2, distribution="gaussian", method="gbm", trContol=control, tuneGrid=gbgrid, metric=metric, allowParallel=TRUE )
```

```{r}
gb_model1 <- gbm(SalePrice~., data=train1, n.trees=100,
                      shrinkage=0.1,
                      n.minobsinnode=c(10),
                      interaction.depth=3, distribution = "gaussian", cv.folds = 10)
```
...
```{r}
xgb_model1 <- train(SalePrice~., data=train1, method="xgbTree", trContol=control, tuneGrid=xgbgrid, metric="RMSE", nthread=3)
```
