...
집이 지어질 때 차고가 같이 지어진 경우 두 변수의 값이 같았다. 그래서 SalePrice 대 GarageYrBlt, SalePrice 대 YearBuilt의 모양이 매우 비슷한 것을 알 수 있었다. 따라서 GarageYrBlt 변수를 분석에서 제외시켰다.
...
변수 중요도를 보면 MSSubClass, OverallQual, NEighbrohood 등 순으로 SalePrice에 영향이 많은 것으로 나타났다. 
...
notselected <- length(which(Implasso$importance$Overall==0))
...
shrinkage는 0.01, n.minobsinnode는 5, interaction.depth는 3, n.trees는 4964일 때 가장 최소 MSE를 가졌다. 따라서 모형을 다시 적합시키면

```{r}
gb_model1 <- gbm(SalePrice~., data=train1, n.trees=4964, 
                 shrinkage=0.01,
                 n.minobsinnode=5,
