...
all <- cbind(all[numvar], categorical_1_hot)
```
...
```{r}
#cv_lasso = cv.glmnet(as.matrix(subset(training, select = -c(SalePrice))), training[,"SalePrice"])

## Predictions
#preds = data.frame(exp(predict(cv_lasso, newx = as.matrix(subset(testing, select = -c(SalePrice))), 
#    s = "lambda.min")) - 1)
```
