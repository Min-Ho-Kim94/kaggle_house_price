...
LotFrontage는 집 앞에 연결된 거리의 크기를 나타내는데,  
```{r}
...
```{r}
NAs(all[,names(all) %in% names(all)[grep("Garage", names(all))]])
...
먼저 "BsmtQual, BsmtCond, BsmtExposure, BsmtFinType1, BsmtFinType2" 5개 변수가 같은 NA를 가지는 행인지 알아보고, 그렇지 않은 행을 추출해보면,
```{r}
##### highlight
...
**BsmtQual**
앞서 정의한 Qualities 벡터 (level을 나타냄)를 이용하여 정수로 releveling하였다. 
...
```{r}
# revalue : ordinal variables
Qualities <- c('None'=0, 'Po'=1, "Fa"=2, "TA"=3, "Gd"=4, "Ex"=5)
Exposure <- c('None'=0, 'No'=1, 'Mn'=2, 'Av'=3, 'Gd'=4)
FinType <- c('None'=0, 'Unf'=1, 'LwQ'=2, 'Rec'=3, 'BLQ'=4, 'ALQ'=5, 'GLQ'=6)

all$PoolQC <- as.integer(revalue(all$PoolQC, replace=Qualities))
all$FireplaceQu <- as.integer(revalue(all$FireplaceQu , replace=Qualities))
...
all$LandContour <- as.integer(revalue(all$LandContour, c("Low"=0, "HLS"=1, "Bnk"=2, "Lvl"=3)))
...
sum(length(chrvar),length(numvar)) #81
```
...
#all <- subset(all, select=-c(TotalBsmtSF, X1stFlrSF, X2ndFlrSF))
```

### character features

아래 4개 변수는 char로 변형해준다.
```{r}
all$MSSubClass = as.character(all$MSSubClass)
all$OverallQual = as.character(all$OverallQual)
all$OverallCond = as.character(all$OverallCond)
...
```{r}
all <- cbind(all[,names(all) %in% numvar], dummies)
dim(all)
```
...
# caret::train()
#\lasso_model <- train(x=training, y=all$SalePrice[!is.na(all$SalePrice)], method = "glmnet", trControl="control", tuneGrid="lassogrid")
```
...
```{r}
rmse(validation0$SalePrice, preds_lasso0) #estimated test mse
```
...
```{r}
write.csv(sub, file="submission.csv", row.names = F)
```


200130 - first (lasso/ no normalizing): scroe 0.12724
