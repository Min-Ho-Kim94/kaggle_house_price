...
## features preprocessing & imputation  {.tabset}
...
#train : 9 / test: 27
all <- all[-which(complete.cases(subset(all,select=-c(SalePrice)))==F) ,]
```

전체 2919개 중 2883개(전체 중 98.77%)이다.


## label encoding features {.tabset}
...
순서형 변수들은 level대로 인코딩해준다. 
```{r}
all$ExterQual<-as.integer(revalue(all$ExterQual,  c("Po"=0, "Fa"=1, "TA"=2, "Gd"=3, "Ex"=4)))
all$ExterCond<-as.integer(revalue(all$ExterCond,  c("Po"=0, "Fa"=1, "TA"=2, "Gd"=3, "Ex"=4)))
all$HeatingQC<-as.integer(revalue(all$HeatingQC,  c("Po"=0, "Fa"=1, "TA"=2, "Gd"=3, "Ex"=4)))
all$KitchenQual<-as.integer(revalue(all$KitchenQual,  c("Po"=0, "Fa"=1, "TA"=2, "Gd"=3, "Ex"=4)))
...
```{r}
dim(all)
all2 <- all[,!(names(all) %in% "SalePrice")]
training <- all2[!is.na(all$SalePrice),]
testing <- all2[is.na(all$SalePrice),]
```
...
```{r}
# glmnet::cv.glmnet()
lasso_model <- cv.glmnet(as.matrix(training), all$SalePrice[!is.na(all$SalePrice)])

preds_lasso <- predict(lasso_model, newx = as.matrix(testing), s="lambda.min")
...
```{r}
#write.csv(preds_lasso, file="submission.csv", row.names = F)
```
